...
[mriT1.name, mriT1.path, mriT1.exists] = uigetfile('*T1*.nii', 'Pick MRI or rMRI file');
...
if c1mri.exists==1 || c1mri.exists==2
 ...
 elseif mriT1.exists==1 ||  mriT1.exists==2
...
%         spm_write_vol(pet.rhdr,pet.rvol);
        ...
        %ictal
        pet.mrhdr = spm_vol(fullfile(pet.path,['mr' pet.name]));
        pet.mrvol = spm_read_vols(pet.mrhdr);
        pet.nanvol = pet.mrvol;
        ...
        pet.hdr = spm_vol(fullfile(pet.path,pet.name));
        pet.vol = spm_read_vols(pet.hdr);
        pet.chdr = pet.hdr;
        pet.cvol = pet.vol ./ rThr .* 50;
        pet.chdr.fname = fullfile(pet.path,['c' pet.name]);
        pet.chdr.private.dat.fname = pet.rchdr.fname;
        pet.chdr.private.dat.dtype = 'FLOAT32-LE'; %'INT16-LE', 'FLOAT32-LE'
        pet.chdr.dt = [16 0]; % 4=16-bit integer; 16=32-bit real datatype

        spm_write_vol(pet.chdr,pet.cvol);
